...
-- Step 2: Add the role column to users table if it doesn't exist
DO $$ 
BEGIN
...
-- Step 3: Drop existing enum if it exists and recreate with new values
DROP TYPE IF EXISTS user_role CASCADE;

-- Step 4: Create new user_role enum with the required roles
CREATE TYPE user_role AS ENUM ('admin', 'driver', 'maintenance_provider');

-- Step 5: Update the users table to use the new enum
-- First, update any existing values to match our new enum values
UPDATE users SET role = 'driver' WHERE role NOT IN ('admin', 'driver', 'maintenance_provider');

-- Now alter the column type
ALTER TABLE users ALTER COLUMN role TYPE user_role USING role::text::user_role;

-- Step 6: Add maintenance_providers table for maintenance provider specific data
...
-- Step 7: Add driver_hours table for tracking driver hours
...
-- Step 8: Add driver_documents table for documents sent to drivers
...
-- Step 9: Add accident_reports table for accident reporting
...
-- Step 10: Verify the migration completed successfully
DO $$
BEGIN
