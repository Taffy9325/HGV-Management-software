-- Simple migration to update existing user_role enum
-- This assumes the role column already exists with a USER-DEFINED type
...
-- Step 2: Check what the current enum values are and update them
-- First, let's see what enum type is currently being used
DO $$
DECLARE
...
BEGIN
    ...
    -- If it's not user_role, we need to handle it differently
    IF current_enum_name != 'user_role' THEN
        RAISE NOTICE 'Converting from % to user_role', current_enum_name;
        
        -- Convert the column to text first
        ALTER TABLE users ALTER COLUMN role TYPE text;
        
        -- Drop the old enum if it exists
        EXECUTE format('DROP TYPE IF EXISTS %I CASCADE', current_enum_name);
    END IF;
END $$;

-- Step 3: Create the new user_role enum with correct values
...
UPDATE users SET role = 'driver' WHERE role NOT IN ('admin', 'driver', 'maintenance_provider');

-- Step 5: Convert the column to use the new enum
ALTER TABLE users ALTER COLUMN role TYPE user_role USING role::text::user_role;

-- Step 6: Add maintenance_providers table
...
-- Step 8: Add driver_documents table
...
-- Step 9: Add accident_reports table
