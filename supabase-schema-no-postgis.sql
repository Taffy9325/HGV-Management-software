-- HGV Compliance Platform Database Schema (Without PostGIS)
-- Use this version if PostGIS is not available in your Supabase project
...
-- Vehicles table (using JSONB for coordinates instead of PostGIS)
CREATE TABLE vehicles (
    ...
    current_location JSONB, -- {lat: number, lng: number} instead of GEOMETRY
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Drivers table (using JSONB for coordinates instead of PostGIS)
...
-- Orders table (using JSONB for coordinates instead of PostGIS)
CREATE TABLE orders (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    ...
    pickup_location JSONB, -- {lat: number, lng: number} instead of GEOMETRY
    delivery_location JSONB, -- {lat: number, lng: number} instead of GEOMETRY
...
-- Create JSONB indexes for location data (alternative to spatial indexes)
...
-- RLS Policies for multi-tenant access (with error handling)
DO $$ BEGIN
