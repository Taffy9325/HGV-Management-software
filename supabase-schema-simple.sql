-- HGV Compliance Platform Database Schema (Simplified - No PostGIS)
...
CREATE TABLE IF NOT EXISTS driver_licence_checks (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    driver_id UUID NOT NULL REFERENCES drivers(id),
...
CREATE TABLE IF NOT EXISTS vehicle_status_checks (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    vehicle_id UUID NOT NULL REFERENCES vehicles(id),
...
-- Examiner packs table
CREATE TABLE IF NOT EXISTS examiner_packs (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_drivers_licence_number ON drivers(licence_number);
CREATE INDEX IF NOT EXISTS idx_orders_tenant_id ON orders(tenant_id);
CREATE INDEX IF NOT EXISTS idx_orders_status ON orders(status);
CREATE INDEX IF NOT EXISTS idx_orders_pickup_window ON orders(pickup_window_start, pickup_window_end);
CREATE INDEX IF NOT EXISTS idx_jobs_tenant_id ON jobs(tenant_id);
CREATE INDEX IF NOT EXISTS idx_jobs_status ON jobs(status);
CREATE INDEX IF NOT EXISTS idx_walkaround_checks_vehicle_id ON walkaround_checks(vehicle_id);
CREATE INDEX IF NOT EXISTS idx_walkaround_checks_completed_at ON walkaround_checks(completed_at);
CREATE INDEX IF NOT EXISTS idx_defects_vehicle_id ON defects(vehicle_id);
...
-- Create JSONB indexes for location data
CREATE INDEX IF NOT EXISTS idx_vehicles_location ON vehicles USING GIN(current_location);
CREATE INDEX IF NOT EXISTS idx_drivers_location ON drivers USING GIN(current_location);
CREATE INDEX IF NOT EXISTS idx_orders_pickup_location ON orders USING GIN(pickup_location);
...
INSERT INTO vehicle_types (name, description, max_weight, max_height, max_length, max_width) 
SELECT 'Rigid Truck', 'Single unit truck', 26000, 4.0, 12.0, 2.55
WHERE NOT EXISTS (SELECT 1 FROM vehicle_types WHERE name = 'Rigid Truck');

INSERT INTO vehicle_types (name, description, max_weight, max_height, max_length, max_width) 
SELECT 'Articulated Truck', 'Tractor unit with trailer', 44000, 4.0, 16.5, 2.55
WHERE NOT EXISTS (SELECT 1 FROM vehicle_types WHERE name = 'Articulated Truck');

INSERT INTO vehicle_types (name, description, max_weight, max_height, max_length, max_width) 
SELECT 'Drawbar Trailer', 'Truck with drawbar trailer', 44000, 4.0, 18.75, 2.55
WHERE NOT EXISTS (SELECT 1 FROM vehicle_types WHERE name = 'Drawbar Trailer');
